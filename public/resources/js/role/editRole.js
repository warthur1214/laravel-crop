...
$(function() {
    ...
    function initRoleInfo(){
        ...
        AjaxJson('/Home/Role/getInfoById/id/' + _id, function( res ){  
            validateForm.assignForm( res );
            initRoleAuthorization( res.organ_id );

            $('#organ_id').val( $('#roleTree [data-id="'+ res.organ_id +'"] > .tree-node').text() );

            initTable({ account_id: res.role_account });  

            var roleOrganTreeInterval = setInterval(function(){ 

                if( $('#roleOrganTree').html() != ''){

                    initRoleCheckbox( $('[name="role_organ"]'), res.role_organ ); 
                    clearInterval(roleOrganTreeInterval);
                }; 
            }, 200);

            var roleModuleTabInterval = setInterval(function(){ 

                if( $('#roleModuleTab .callout').length > 0 ){ 
                    
                    initRoleCheckbox( $('[name="role_module"]'), res.role_module ); 
                    clearInterval( roleModuleTabInterval );
                }; 
            }, 200); 
    ...
    $('#submit').bind('click', function(){
    	...
    	if( validateForm.validnew() ){
            ...
            postdata.role_id = _id;

    		AjaxJson('/Home/Role/editRoleAjax', postdata, function( res ){
    ...
    $('#AddRoleAccount').bind('click', function(){  //点击添加本角色账号
