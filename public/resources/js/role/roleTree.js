    ...
    var _initOrgTree = {
        init: function (opts) {
            ...
            AjaxJson(me.url, function (res) {
                if (res) {
                    ...
                    me.$el.html('');
                    me.renderList(res);
                    me.$el.html('<ul class="' + me.className + '"><li data-id="">' + _checkbox + '<span class="tree-node">全部</span></li>' + _html + '</ul>');
                    me.bindEvent();
                }
        ...
        renderList: function (res) {
            ...
            for (var i = 0, l = res.length; i < l; i++) {
                ...
                if (me.checkbox) {
                    _checkbox = '<input type="checkbox" value="' + _d[_val] + '" name="' + _checkboxName + '" /> ';
                }

                if (_d[_children]) {
                    _html += '<li class="tree-parent" data-id="' + _d[_val] + '"><i class="fa fa-collapse fa-chevron-down"></i>' + _checkbox + '<span class="tree-node">' + _d[_text] + '</span><ul>';
                    me.renderList(_d.son);
                    _html += '</ul></li>';
                } else {
                    _html += '<li class="tree-leaf" data-id="' + _d[_val] + '">' + _checkbox + '<span class="tree-node">' + _d[_text] + '</span></li>';
                }
